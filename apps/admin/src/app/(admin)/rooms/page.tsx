'use client';

import { useCallback, useEffect, useState } from 'react';
import { api, getApiErrorMessage } from '@/lib/api';
import type { ApiResponse, VoiceRoom } from '@/lib/types';
import { PageSkeleton } from '@/components/ui/skeleton';
import { ErrorState } from '@/components/ui/error-state';
import { EmptyState } from '@/components/ui/empty-state';

export default function RoomsPage() {
  const [rooms, setRooms] = useState<VoiceRoom[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const { data } = await api.get<ApiResponse<VoiceRoom[]>>('/rooms');
      setRooms(data.data);
    } catch (err) {
      setError(getApiErrorMessage(err));
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    void load();
  }, [load]);

  if (loading) return <PageSkeleton />;
  if (error) return <div className="p-8"><ErrorState message={error} onRetry={load} /></div>;

  const active = rooms.filter((r) => r.status === 'active');

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold">Voice Rooms</h1>
        <p className="text-[var(--text-secondary)] mt-1">Active voice rooms from API</p>
      </div>

      <div className="glass-card p-5 mb-6 inline-block">
        <p className="text-2xl font-bold text-cyan-400">{active.length}</p>
        <p className="text-sm text-[var(--text-secondary)]">Active Rooms</p>
      </div>

      {rooms.length === 0 ? (
        <EmptyState message="No voice rooms yet" />
      ) : (
        <div className="glass-card overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5 text-[var(--text-secondary)]">
                <th className="text-left p-4">Title</th>
                <th className="text-left p-4">Category</th>
                <th className="text-left p-4">Listeners</th>
                <th className="text-left p-4">Status</th>
              </tr>
            </thead>
            <tbody>
              {rooms.map((room) => (
                <tr key={room.id} className="border-b border-white/5">
                  <td className="p-4 font-medium">{room.title}</td>
                  <td className="p-4">{room.category}</td>
                  <td className="p-4">{room.listenerCount}</td>
                  <td className="p-4">{room.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
